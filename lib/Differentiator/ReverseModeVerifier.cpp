...
namespace clad {
  ...
  void ReverseModeVerifier::ActOnEndOfDerivedFnBody(const DiffRequest& request,
    ...
    for (const auto& arg : args) {
      ...
      auto fileNameLiteral = utils::CreateStringLiteral(C,
                                                        baseFileName);
      auto fnNameLiteral = utils::CreateStringLiteral(C,
                                                      qualifiedGradientName);
      auto assertMessageLiteral = utils::
          CreateStringLiteral(C, assertMessage);
      ...
      FunctionDecl*
          verifyResultFD = getCladVerifyResultSpecialization(m_RMV->getSema(),
                                                             verifyResultArgs);
      CXXScopeSpec CSS;
      CSS.Extend(C, m_RMV->GetCladNamespace(), noLoc,
                 noLoc);
