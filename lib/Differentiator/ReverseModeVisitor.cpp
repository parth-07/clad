...
namespace clad {
  ...
  StmtDiff ReverseModeVisitor::VisitSwitchStmt(const SwitchStmt* SS) {
    ...
    if (SS->getInit()) {
      ...
      addToCurrentBlock(switchInitDiff.getStmt(), forward);
      addToCurrentBlock(switchInitDiff.getStmt_dx(), reverse);
    }

    // Handles condition variable
    if (SS->getConditionVariable()) {
      StmtDiff condVarDiff = DifferentiateSingleStmt(SS->getConditionVariableDeclStmt());
      addToCurrentBlock(condVarDiff.getStmt(), forward);
      addToCurrentBlock(condVarDiff.getStmt_dx(), reverse);
    }
    ...
    if (!(SSData->cases.empty())) {
      Sema::ConditionResult
          condRes = m_Sema.ActOnCondition(m_CurScope, noLoc, condExpr,
      ...
      if (isInsideLoop) {
        addToCurrentBlock(condTape->Pop, reverse);
      }
    ...
    PopSwitchStmtInfo();
    return {endBlock(forward), endBlock(reverse)};
  }
  ...
  StmtDiff ReverseModeVisitor::VisitCaseStmt(const CaseStmt* CS) {
    ...
    addToCurrentBlock(ifBreakExpr, reverse);
    addToCurrentBlock(newSC, forward);
    auto diff = DifferentiateSingleStmt(CS->getSubStmt());
    utils::SetSwitchCaseSubStmt(newSC, diff.getStmt());
    addToCurrentBlock(diff.getStmt_dx(), reverse);
    return {endBlock(forward), endBlock(reverse)};
  }
  ...
  StmtDiff ReverseModeVisitor::VisitDefaultStmt(const DefaultStmt* DS) {
    ...
    addToCurrentBlock(ifBreakExpr, reverse);
    addToCurrentBlock(newDefaultStmt, forward);
