...
namespace clad {
  ...
  FunctionDecl* ReverseModeVisitor::CreateGradientOverload() {
    ...
    bool isInstanceMethod = utils::IsInstanceMethod(m_Function);
    ...
    std::size_t numOfDerivativeParams =
        m_Function->getNumParams() + isInstanceMethod;
  ...
  ReverseModeVisitor::DerivePullback(const clang::FunctionDecl* FD,
    ...
    bool isInstanceMethod = utils::IsInstanceMethod(FD);

    assert((!args.empty() || isInstanceMethod) &&
  ...
  StmtDiff ReverseModeVisitor::VisitCallExpr(const CallExpr* CE) {
    ...
    if (!NArgs && !dyn_cast<CXXMemberCallExpr>(CE))
      return StmtDiff(Clone(CE));
